...
describe("createMessageTool", () => {
  ...
  test("sends message to chat", async () => {
    const tool = createMessageTool(chatManager, "sess_1", "agent", "agent")
  ...
  test("sends as npc with correct identity", async () => {
    ...
    const tool = createMessageTool(chatManager, "sess_1", "npc_1", "npc")
  ...
  test("calls onMessage callback", async () => {
    ...
    const tool = createMessageTool(chatManager, "sess_1", "agent", "agent", (c) =>
      received.push(c),
    )
