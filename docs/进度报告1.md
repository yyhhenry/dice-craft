...
所有 Skill 和游戏设计文档并非开发者提前编写的流程和脚本，而是来自实际游戏过程中对话式的交互创建和调整。用户只给出粗略想法，Builder Agent 自行编写脚本和 SKILL.md，将其封装为可复用的游戏编排。

运行时由 GM 通过 JSON 持久化维护游戏状态，避免大模型直接计算产生的错误。用户在试玩中可以随时打断，回到编辑状态，继续调整打磨游戏设计。

其中有复用价值的 Skill 被复制到本项目 `templates/` 中，作为后续新建 workspace 时的自带 Skill。这验证了一个正向循环：**游戏过程产出工具，好工具丰富后续游戏的默认能力**。
